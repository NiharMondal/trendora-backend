 ...
 */
const nullableFieldCache = new Map<string, Set<string>>();
...
const nullableFieldsFor = (model: string): Set<string> => {
    ...
    search(
        ...
        for (const path of relationPaths) {
            ...
            if (segments.length < 2 || segments.some((segment) => !segment)) {
                throw new Error(
                    `search(): relation path "${path}" must be "relation.column" or deeper`,
                );
            }
            ...
            conditions.push(
                segments.reduceRight<Record<string, unknown>>(
                    (inner, relation) => ({ [relation]: { is: inner } }),
                    { [column]: match },
                ),
            );
        }
