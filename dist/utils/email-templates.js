...
exports.payoutPaidEmail = exports.vendorApplicationDecisionEmail = exports.refundProcessedEmail = exports.orderStatusEmail = exports.newOrderForSellerEmail = exports.orderPlacedEmail = exports.passwordChangedEmail = exports.passwordResetEmail = void 0;
...
const orderPlacedEmail = ({ name, orderNumber, totalAmount, paymentMethod, groups, }) => ({
    ...
    html: layout(`Order ${orderNumber} confirmed`, `
    <p style="margin:0 0 16px;font-size:14px;line-height:22px;color:#374151;">Hi ${name},</p>
    <p style="margin:0 0 24px;font-size:14px;line-height:22px;color:#374151;">
...
const newOrderForSellerEmail = ({ storeName, orderNumber, parcelNumber, items, earning, }) => ({
    ...
    html: layout("You have a new order", `
    <p style="margin:0 0 24px;font-size:14px;line-height:22px;color:#374151;">
...
exports.newOrderForSellerEmail = newOrderForSellerEmail;
/**
 * Buyer: this parcel moved. Only sent for states the buyer cares about —
 * SHIPPED, DELIVERED and CANCELED. PROCESSING is noise.
 */
const orderStatusEmail = ({ name, orderNumber, parcelNumber, storeName, status, trackingNumber, carrier, cancelReason, refundExpected, }) => {
    const headline = status === "SHIPPED"
    ...
    if (trackingNumber)
        detail.push(["Tracking number", trackingNumber]);
    if (carrier)
        detail.push(["Carrier", carrier]);
    if (status === "CANCELED" && cancelReason)
        detail.push(["Reason", cancelReason]);
    const refundNote = status === "CANCELED" && refundExpected
...
exports.orderStatusEmail = orderStatusEmail;
/** Buyer: money actually went back. Sent only once a refund SUCCEEDS. */
const refundProcessedEmail = ({ name, orderNumber, amount, }) => ({
    ...
    html: layout("Your refund is on its way", `
    <p style="margin:0 0 16px;font-size:14px;line-height:22px;color:#374151;">Hi ${name},</p>
    ${rows([
        ["Order", orderNumber],
...
const vendorApplicationDecisionEmail = ({ name, storeName, approved, rejectionReason, storeUrl, }) => approved
    ? {
        ...
        html: layout(`${storeName} is approved`, `
    <p style="margin:0 0 16px;font-size:14px;line-height:22px;color:#374151;">Hi ${name},</p>
    <p style="margin:0 0 24px;font-size:14px;line-height:22px;color:#374151;">
    ...
    : {
        ...
        html: layout("Your store application was not approved", `
    <p style="margin:0 0 16px;font-size:14px;line-height:22px;color:#374151;">Hi ${name},</p>
...
exports.vendorApplicationDecisionEmail = vendorApplicationDecisionEmail;
/** Seller: a payout has been sent. */
const payoutPaidEmail = ({ name, storeName, amount, reference, method, }) => {
    const detail = [
        ["Store", storeName],
        ["Amount", money(amount)],
    ];
    if (method)
        detail.push(["Method", method]);
    if (reference)
        detail.push(["Reference", reference]);
    return {
        ...
        html: layout("Your payout has been sent", `
    <p style="margin:0 0 16px;font-size:14px;line-height:22px;color:#374151;">Hi ${name},</p>
    ${rows(detail)}`),
    };
};
exports.payoutPaidEmail = payoutPaidEmail;
