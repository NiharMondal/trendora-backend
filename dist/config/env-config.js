...
exports.warnAboutDisabledFeatures = exports.envConfig = void 0;
 ...
 */
const numeric = (label, fallback) => zod_1.z
    .string()
    .optional()
    .transform((value, ctx) => {
...
const envSchema = zod_1.z.object({
    ...
    PLATFORM_COMMISSION_RATE: numeric("PLATFORM_COMMISSION_RATE", 0.10)
        .refine((n) => n >= 0 && n <= 1, "PLATFORM_COMMISSION_RATE must be between 0 and 1 (0.10 = 10%)"),
    ...
    SEED_PASSWORD: zod_1.z.string().default("Password123!"),
 ...
 */
const warnAboutDisabledFeatures = () => {
    const disabled = [];
    if (!exports.envConfig.stripe_secret_key)
        disabled.push("Stripe payments (STRIPE_SECRET_KEY)");
    if (!exports.envConfig.stripe_webhook_secret)
        disabled.push("Stripe webhook verification (STRIPE_WEBHOOK_SECRET)");
    if (!exports.envConfig.cloudinary.cloud_name || !exports.envConfig.cloudinary.api_secret)
        disabled.push("Cloudinary image handling (CLOUD_NAME / API_KEY / API_SECRET)");
    if (!exports.envConfig.emailUtils.email || !exports.envConfig.emailUtils.password)
        disabled.push("Transactional email (EMAIL / PASSWORD)");
    if (disabled.length > 0) {
        console.warn(`[config] running with these features DISABLED:\n${disabled
            .map((item) => `  - ${item}`)
            .join("\n")}`);
    }
};
exports.warnAboutDisabledFeatures = warnAboutDisabledFeatures;
